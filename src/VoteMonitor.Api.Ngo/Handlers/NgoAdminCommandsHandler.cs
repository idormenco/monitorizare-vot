...
namespace VoteMonitor.Api.Ngo.Handlers
{
...

    {
        ...
        public NgoAdminCommandsHandler(ILogger logger, IMapper mapper, VoteMonitorContext context, IHashService hashService)
        {
            _logger = logger;
            _mapper = mapper;
            _context = context;
        ...
        public async Task<Result> Handle(UpdateNgoAdmin request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                ngoAdmin.Password = request.NgoAdmin.Password; // TODO: use hashing service
